...
public class AStar {
	...
	public static void aStar() {
		...
		while (openList.isEmpty()!=false) {
			...
			openList.remove(current);// f i en küçük olaný seçtik ve open list
										// den kaldýrýp closed list e ekledik.
			closedList.add(current);
			// Þimdi bu current in sahip olduðu komþularý bul
			for (int i = 0; i < closedList.size(); i++) {
				...
				for (int i = 0; i < komsu.size(); i++){
					for (int j = 0; j <openList.size() ; j++) {
						if (komsu.get(i) == openList.get(j)) {
							System.out.println("bu komþu openlistte\n"+komsu.get(i));
							durum=1;
							break;
						}
					}
					if(durum==0){
						...
						}//eðer open list e ekli deðilse open list e ekledik.
